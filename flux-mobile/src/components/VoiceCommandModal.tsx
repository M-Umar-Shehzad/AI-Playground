...
export default function VoiceCommandModal({ visible, onClose, onSend, userCountry }: VoiceCommandModalProps) {
    ...
    const processAudio = async (uri: string) => {
        try {
            ...
            // Priority 1: Use window origin if deployed on web (fixes Replit/Render port mismatch)
            if (Platform.OS === 'web' && typeof window !== 'undefined') {
                hostUrl = window.location.origin;
            }
            // Priority 2: Use explicit environment API URL
            else if (process.env.EXPO_PUBLIC_API_URL) {
                hostUrl = process.env.EXPO_PUBLIC_API_URL;
            }
            // Priority 3: Fallback to local Expo host (for purely mobile native testing)
