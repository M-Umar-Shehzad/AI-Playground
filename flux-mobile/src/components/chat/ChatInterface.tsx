...
export default function ChatInterface({ user, userCountry, onLoginRequest, onTransactionSaved, onGuestTransaction, onOfflineTransaction, externalInputText }: ChatInterfaceProps) {
    ...
    const handleSubmit = () => {
        ...
        // Network Interception (Phase 19)
        if (isOffline) {
            // Manually append the user's message since we bypass the SDK's sendMessage
            ...
            if (parsed.length === 0) {
                ...
                setMessages((prev: any) => [...prev, {
                    id: astMsgId,
                    role: "assistant", // Using the correct type based on your interface
        ...
        // Online mode: let SDK handle both the user message appending and the API call natively
        sendMessage({ text });
    };
